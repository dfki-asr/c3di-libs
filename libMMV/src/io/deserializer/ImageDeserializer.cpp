...
namespace ettention
{
    ...
    Image* ImageDeserializer::readImageFromStack(std::string filename, unsigned int index)
    {
        ...
        if(!imageFile.open(filename.c_str(), false, true))
            throw std::ios_base::failure("unable to open image stack " + filename);

        unsigned int numberOfImages = imageFile.getPageCount();
        if(index > numberOfImages)
            throw std::range_error("index out of bound: image stack does not have that many images");

        fipImage fimg;
        fimg = imageFile.lockPage(index);
        if(!fimg.isValid())
            throw std::ios_base::failure("failed to load image from stack");

...
namespace ettention
{
    ...
    float StatisticalStandardMeasures::computeMeanUsingMask(const float* data, const byte_t* mask, byte_t flag, size_t numberOfElements)
    {
        float meanValue = 0.0f;
        auto numberOfAffectedElements = 0;
        ...
        return (meanValue / numberOfAffectedElements);
    }
    ...
    float StatisticalStandardMeasures::computeVarianceUsingMask(const float* data, const byte_t* mask, byte_t flag, size_t numberOfElements, bool unbiasedEstimate)
    {
        float sdValue = 0.0f;
        float meanValue = computeMeanUsingMask(data, mask, flag, numberOfElements);
        auto numberOfAffectedElements = 0;
        ...
        if( unbiasedEstimate )
        {
            sdValue /= (numberOfAffectedElements - 1);
        } else {
            sdValue /= numberOfAffectedElements;
        }
